...
func (h *Handler) Login(w http.ResponseWriter, r *http.Request) {
	...
	// If company exists, prefer subscription_plans via subscriptions table
	if err == nil {
		var subPlan string
		subErr := h.db.Pool().QueryRow(ctx, `
			SELECT sp.name
			...
			LIMIT 1
		`, storeID).Scan(&subPlan)
		if subErr == nil && subPlan != "" {
			plan = subPlan
			// Sync company subscription_plan to the active subscription
			_, _ = h.db.Pool().Exec(ctx, `UPDATE companies SET subscription_plan = $1 WHERE id = $2`, subPlan, storeID)
		}
	}
