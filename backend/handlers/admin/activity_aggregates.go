...
// GetActivityAggregates returns aggregated activity events with filters.
// Filters: company_id, user_id, action_type, start_date, end_date. Pagination supported.
func (h *AdminHandler) GetActivityAggregates(w http.ResponseWriter, r *http.Request) {
	ctx := r.Context()

	var companyID, userID, actionType *string
	...
	// Build query
	query := `
		SELECT date, user_id, company_id, action_type, count
	...
	if actionType != nil {
		query += ` AND action_type = $` + strconv.Itoa(arg)
	...
	if err != nil {
		h.respondError(w, errors.NewDatabaseError(err, "query activity aggregates"), r)
		return
	}
	defer rows.Close()
	...
	for rows.Next() {
		var rec row
		if err := rows.Scan(&rec.Date, &rec.UserID, &rec.CompanyID, &rec.ActionType, &rec.Count); err == nil {
			data = append(data, rec)
		}
	}

	// Count query
	countQuery := `SELECT COUNT(*) FROM activity_aggregates WHERE 1=1`
	countArgs := []interface{}{}
	carg := 1
	if companyID != nil {
	...
	if actionType != nil {
		countQuery += ` AND action_type = $` + strconv.Itoa(carg)
